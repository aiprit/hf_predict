...
def init_params(options):
	...
	with open(options['embFile'], 'rb') as f:
		params['W_emb'] = np.array(pickle.load(f, encoding='latin1')).astype(config.floatX)
	print (params['W_emb'])
...
def load_data(seqFile, labelFile, timeFile=''):
	...
	if len(timeFile) > 0:
		...
		with open(timeFile, 'rb') as f:
			times = np.array(pickle.load(f, encoding='latin1'))

	dataSize = len(labels)
	ind = np.random.permutation(dataSize)
	nTest = int(0.10 * dataSize)
	nValid = int(0.40 * dataSize)
...
if __name__ == '__main__':
	...
	train_GRU_RNN(dataFile=dataFile, labelFile=labelFile, timeFile=timeFile, embFile=embFile, outFile=outFile, inputDimSize=inputDimSize, embDimSize=embDimSize, hiddenDimSize=hiddenDimSize, max_epochs=max_epochs, L2_reg=L2_reg, batchSize=batchSize, use_dropout=use_dropout)
